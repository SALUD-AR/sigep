...
variables_form_busqueda("listado_hist_4");

//$fecha_hoy=date("Y-m-d H:i:s");
//$fecha_hoy=fecha($fecha_hoy);
$fecha_hoy=date("Y-m-d");
$plazo_29=date("Y-m-d", strtotime ("$fecha_hoy +29 days"));
$plazo_10=date("Y-m-d", strtotime ("$fecha_hoy +10 days"));
...
$sql_tmp="select * from expediente.expediente left join (select id_efe_conv,nombre,cuie from nacer.efe_conv) as efector
using (id_efe_conv) where plazo_para_pago between '$plazo_10' and '$plazo_29'";
...
echo $html_header;
?>
<form name=form1 action="historial_4.php" method=POST>
...
<?
    ...
    if ($result->fields['comentario1']=="Expediente Pago, en Archivo"){ $tr=atrib_tr3();}
    else { 
    $tr=atrib_tr1();?>
