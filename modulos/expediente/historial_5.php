...
variables_form_busqueda("listado_hist_5");
...
select id_expediente,num_tranf from expediente.transaccion where num_tranf is not null) as tabla
left join expediente.expediente using (id_expediente)
where control=5 or comentario1='$string_coment' order by id_expediente DESC) as tabla2 left join nacer.efe_conv using (id_efe_conv)";
/*$sql_tmp="select * from (SELECT * FROM expediente.expediente left join nacer.efe_conv using (id_efe_conv) where control=5 or comentario1='$string_coment')
 as tabla1";*/
       
echo $html_header;
?>
<form name=form1 action="historial_5.php" method=POST>
...
<tr>
    <td align=right id=mo><a id=mo href='<?=encode_link("historial_det.php",array("sort"=>"1","up"=>$up))?>'>Numero de Expediente</a></td>
     ...
     <td align=right id=mo><a id=mo >Nro.Factura</a></td>	 
    <td align=right id=mo><a id=mo >Fecha de Ingreso</a></td> 
    <td align=right id=mo><a id=mo>Fecha de Plazo</a></td>
    <td align=right id=mo><a id=mo >Periodo</a></td> 
    <td align=right id=mo><a id=mo >Monto</a></td>
    <td align=right id=mo><a id=mo >Expediente Externo</a></td>
