...
variables_form_busqueda("listado_hist_3");
...
//$sql_tmp="select * from (select id_expediente from expediente.transaccion where estado='R') as tabla1 left join expediente.expediente using (id_expediente) left join nacer.efe_conv using (id_efe_conv)";
$sql_tmp="select * from (
select * from (
select * from expediente.expediente where estado='R' ) as expediente
left join (select id_expediente,fecha_mov,estado as est_tran from expediente.transaccion) as transaccion 
using (id_expediente) where est_tran='R')as completo
...
echo $html_header;
?>
<form name=form1 action="historial_3.php" method=POST>
