...
if($cuie!='Todos'){
			$sql_tmp="SELECT 
						...
						where (fichero.fichero.fecha_control BETWEEN '$fecha_desde' and '$fecha_hasta') and (nacer.efe_conv.cuie='$cuie') and (embarazo='SI')
				...
				$sql_tmp="SELECT 
							...
							nacer.efe_conv.nombre as nom_efe
								...
								LEFT OUTER JOIN leche.beneficiarios ON leche.beneficiarios.id_beneficiarios = fichero.fichero.id_beneficiarios
								INNER JOIN nacer.efe_conv ON nacer.efe_conv.cuie = fichero.fichero.cuie
							where (fichero.fichero.fecha_control BETWEEN '$fecha_desde' and '$fecha_hasta') and (embarazo='SI')
							ORDER BY fecha_control";
							
			}
			
$res_comprobante=sql($sql_tmp,"<br>Error al traer los datos<br>") or fin_pagina();
excel_header("report_fichero_nomi_emb_excel.xls");?>

<form name=form1 action="report_fichero_nomi_emb_excel.php" method=POST>
<table border=0 width=100% cellspacing=2 cellpadding=2 align=center>
  <tr>
    ...
    <td align=right>Fecha Control</td>
    <td align=right>Peso</td>
    <td align=right>Talla</td>
    <td align=right>IMC</td>
    <td align=right>Per. Cef.</td>
    <td align=right>Perc Peso/Edad</td>
    <td align=right>Perc Talla/Edad</td>
    <td align=right>Perc Perim. Cefalico/Edad</td>
    <td align=right>Perc IMC/Edad</td>
    <td align=right>Perc Peso/Talla</td>
    <td align=right>Fecha Diag</td>
    <td align=right>FUM</td>
    <td align=right>FPP</td>
    <td align=right>Sem Ges</td>
