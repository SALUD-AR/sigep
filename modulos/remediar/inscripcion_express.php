...
if ($_POST['guardar_editar']=="Guardar"){
  ...
  if($responsable =='MADRE'){
             ...
             indigena=upper('$indigena'),id_tribu=0,id_lengua=0,
	...
	if (($id_categoria=='1') && ($estado_envio== 'n'))  {
             ...
             apellido_benef=upper('$apellido'),
             numero_doc='$num_doc',clase_documento_benef=upper('$clase_doc'),fecha_nacimiento_benef='$fecha_nac',
             indigena=upper('$indigena'),id_tribu=0,id_lengua=0,id_categoria=$id_categoria,
...
if (($res_extra->recordcount()>0) && ($responsable=='TUTOR')) {
   ...
   if (($res_extra->recordcount()>0) && ($id_categoria == '1')){
      	$query="insert into uad.beneficiarios
             ...
             ($id_planilla,'n','$clave_beneficiario',upper('$tipo_transaccion'),upper('$apellido'),upper('$nombre'),upper('$clase_doc'),
             upper('$tipo_doc'),'$num_doc','$id_categoria',upper('$sexo'),'$fecha_nac',upper('$provincia_nacn'),upper('$localidad_procn'),upper('$pais_nacn'),
             upper('$indigena'),0,0,'$fecha_diagnostico_embarazo','$semanas_embarazo',
 ...
 if ((document.all.id_categoria.value=='3')||(document.all.id_categoria.value=='4')){
			...
			var anio_mayor_nivel=document.all.anio_mayor_nivel.value;
			 ...
			 if(isNaN(anio_mayor_nivel) || isNaN(anio_mayor_nivel_madre) ){
				alert('El dato ingresado en a�os mayor nivel debe ser entero');
			 return false;
			 }
...
<form name='form1' action='inscripcion_express.php' accept-charset="latin1" method='POST'>
		...
		<tr>
         ...
         <tr>
		    ...
		    <td align="left" colspan="2">
		    	 <input type=text name=fecha_inscripcion id=fecha_inscripcion onblur="esFechaValida(this);" value='<?=$fecha_inscripcion;?>' size=15 onkeypress="return pulsar(event)" >
				 <?php if (!$id_planilla) {?>
...
<table class="bordes" align="center" width="100%">
		 ...
		 <tr>
		    <td align="center">
		      ...
		      <? $ref = encode_link("remediar_carga_rapida.php",array("estado_envio"=>$estado_envio,"clave_beneficiario"=>$clave_beneficiario,"sexo"=>$sexo,"fecha_nac"=>$fecha_nac,"vremediar"=>'s',"pagina"=>'inscripcion_express.php'));?>
		      <input type=button name="carga_remediar_rapida" value="Clasificacion Rapida Redes" onclick="window.open('<?=$ref?>','Clasificacion','menubar=1,resizable=1,scrollbars=1,width=1000,height=750');" title="Clasificacion Rapida" <?if ($tipo_transaccion == "B") echo "disabled"?>>&nbsp;
		      
		      <input type=button name="carga_remediar" value="Calculo Score Redes" onclick="window.open('<?=encode_link("../remediar/remediar_admin.php",array("estado_envio"=>$estado_envio,"clave_beneficiario"=>$clave_beneficiario,"sexo"=>$sexo,"fecha_nac"=>$fecha_nac,"vremediar"=>'s',"pagina_viene_1"=>"ins_admin_old.php"))?>','Remediar','dependent:yes,width=1000,height=700,top=1,left=60,scrollbars=yes');" title="Carga Remediar + Redes" <?if ($tipo_transaccion == "B") echo "disabled"?>>&nbsp;
		    </td>
		 </tr> 
	 </table>	
	 <br>
 ...
 <tr><td><table width=100% align="center" class="bordes">
  <tr align="center">
   <td>
 	...
 	<?}ELSE{?>
  	<input type=button name="volver" value="Volver" onclick="document.location='listado_clasificados.php'"title="Volver al Listado" style="width=150px">     
 	<?}?>  
   </td>
  </tr>
