...
//detalle de facturas ingresadas en una determinada fecha	
		    ...
		    $sql_comprobantes="select nomenclador.grupo, nomenclador.codigo, con1.fila, nomenclador.descripcion, nomenclador_detalle.descripcion as descripcion_nomenclador , con1.cantidad, nomenclador.precio, (con1.cantidad*nomenclador.precio) as total
								from(
									select nomenclador.id_nomenclador, nomenclador.grupo||nomenclador.codigo||prestacion.diagnostico||nomenclador.id_nomenclador as fila , count (nomenclador.grupo||nomenclador.codigo||prestacion.diagnostico||nomenclador.id_nomenclador) as cantidad
																	from facturacion.factura
									...
									GROUP BY nomenclador.id_nomenclador,nomenclador.grupo||nomenclador.codigo||prestacion.diagnostico||nomenclador.id_nomenclador
									) as con1
								inner join facturacion.nomenclador using (id_nomenclador)
...
<form name='form1' action='efectores_detalle_solo_sps.php' method='POST'>
...
<table width="100%" cellspacing=0 border=1 bordercolor=#E0E0E0 align="center" bgcolor='<?=$bgcolor_out?>' class="bordes">		 
		 <tr align="center" id="sub_tabla">
		 	<td colspan=10>	
		 		Detalle sobre Facturas LIQUDADAS CERRADA (no toma en cuenta el Ingreso al Sistema de Expedientes)
		 	</td>
		 </tr>
...
<table width="100%" cellspacing=0 border=1 bordercolor=#E0E0E0 align="center" bgcolor='<?=$bgcolor_out?>' class="bordes">		 
		 <tr align="center" id="sub_tabla">
		 	<td colspan=7>	
		 		Detalle sobre Prestaciones de Facturas LIQUDADAS CERRADA (no toma en cuenta el Ingreso al Sistema de Expedientes)
		 	</td>
		 </tr>
		 
		 <tr>
			<td align=right id=mo><a id=mo>Practica</a></td> 
			<td align=right id=mo><a id=mo>Codigo</a></td> 
			<td align=right id=mo><a id=mo>Diagnostico</a></td> 
			<td align=right id=mo><a id=mo>Completo</a></td> 
			<td align=right id=mo><a id=mo>Descripcion</a></td>  
			<td align=right id=mo><a id=mo>Descripcion Diag</a></td>
 ...
 <?
  ...
  while (!$res_comprobantes->EOF) {
  	?>
    <tr <?=atrib_tr1()?>>   
     <td align=left><?=$res_comprobantes->fields['grupo']?></td>       
     <td align=left><?=$res_comprobantes->fields['codigo']?></td>
     <td align=left><?=substr ($res_comprobantes->fields['fila'],6,3)?></td>
     <td align=left><?=substr ($res_comprobantes->fields['fila'],0,9)?></td>     
     <td align=left><?=$res_comprobantes->fields['descripcion']?></td>
     <?$diag_cons=substr ($res_comprobantes->fields['fila'],6,3);
     $sql="select descripcion from nomenclador.patologias_frecuentes where codigo = '$diag_cons';";
     $res_comprobantes_diag=sql ($sql,"error diag") or die;
     ?>
     <td align=left><?=$res_comprobantes_diag->fields['descripcion']?></td>
	...
	<?$res_comprobantes->MoveNext();
  	...
  	<tr>   
     <b><font size="3" color="red"><td colspan=4 align= left>Cantidad: <?=$res_comprobantes->RecordCount();?></td></font></b>    
    </tr>
  <?}?>   
</table>
<?}?>
