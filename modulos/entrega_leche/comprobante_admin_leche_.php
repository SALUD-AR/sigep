  ...
  $res_qcos=sql($query_cos,"Error al realizar verificacion de entregas anteriores") or fin_pagina();
	if ($res_qcos->RecordCount()==EOF){		
				...
				$query="insert into leche.info_social
				        (id_informe,id_smiafiliados,id_beneficiarios,cuie,resp_infor,autorizado,fecha_inf,informe, nro_exd)
				        values
				        ($id_informe,'$id_smiafiliados','$id_beneficiarios','$efector_informe','$resp_infor','PEND','$fecha_inf','$informe', '$nro_exd')";	
	 	...
	 	$res_comprobante->movefirst();
            ...
            $onclick_marcar="if (confirm('Esta Seguro que Desea Eliminar?')) location.href='$ref1'
	 		...
	 		<tr <?=atrib_tr()?>>	 			
		 		...
		 		<td><?=$res_comprobante->fields['nro_exd']?></td>
				          ...
				          <td colspan=9>					  
				           <div id=<?=$id_tabla?> style='display:none'>
				            <table width=100% align=center >
							     ...
							     <td align="left">  						    	
									<input type='text' name='nro_exd' value='<?=$nro_exd?>' size=30 align='right' <? if($id_informe) echo "disabled"?>></b>				    	 				    	 
							  	</td>
							 </tr>  
	         ...
	         <?/*----------tercero datos---------------*/
		        if($resp_dos->RecordCount()!=0){
				  ...
				  <tr> 
					...
					<td align=right id=mo title="Guardar Modificacion">Cargar</td>
	  			</tr>
 				<? while (!$resp_dos->EOF) {
					    ...
					    <tr <?=atrib_tr()?>> 
							...
							<? $ref1=encode_link("comprobante_nuevo_archivo.php",array("guardar_info_ed"=>"Guardar","id_informe"=>$resp_dos->fields['id_informe'],'id'=>$id,'entidad_alta'=>$entidad_alta)); 
					 		$onclick_guardar="if (confirm('Desea Guardar los cambios?')) location.href='$ref1'"; ?>
					 		<td><img src='../../imagenes/menur/iconSave.gif' style='cursor:hand;' onclick="<?=$onclick_guardar?>"></td>
							<?}else echo " &nbsp";?>	
						</tr>
